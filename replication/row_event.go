...
func (e *TableMapEvent) String() string {
	var s = "{TableIDSize:" + fmt.Sprint(e.TableIDSize) +
		" TableID:" + fmt.Sprint(e.TableID) +
		" Flags:" + fmt.Sprint(e.Flags) +
		" Schema:" + string(e.Schema) +
		" Table:" + string(e.Table) +
		" ColumnCount:" + fmt.Sprint(e.ColumnCount)
	var tps = make([]string, len(e.ColumnType))
	for i, v := range e.ColumnType {
		tps[i] = fmt.Sprint(v)
	}
	s += " ColumnType:" + strings.Join(tps, ", ")
	s += " ColumnTypeLength:" + fmt.Sprint(len(e.ColumnType))
	var mts = make([]string, len(e.ColumnMeta))

	for i, v := range e.ColumnMeta {
		mts[i] = fmt.Sprint(v)
	}
	s += " ColumnMeta:" + strings.Join(mts, ", ")
	s += " ColumnMetaLength:" + fmt.Sprint(len(e.ColumnMeta))
	s += " NullBitmap:" + string(e.NullBitmap) + "}"
	return s
}
